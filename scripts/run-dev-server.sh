...
vagrant ssh -c "fleetctl start ircdd/scripts/dev-config/service/nsqadmin@first.service"
...
vagrant ssh -c "fleetctl start ircdd/scripts/dev-config/service/nsqlookupd@search.service"
